...
async function updateSettings(path) {
  ...
  clientToUse =
    settings.cliOptions?.clientToUse || settings.clientToUse || "bing";
  // console.log(settings)

  clientOptions = getClientSettings(clientToUse, settings);
  client = getClient(clientToUse, settings);

  // Ensure messageOptions is initialized
  clientOptions.messageOptions = clientOptions.messageOptions || {};

  // If there's a system message in the settings, use it
  if (settings[`${clientToUse}Options`]?.messageOptions?.systemMessage) {
    clientOptions.messageOptions.systemMessage =
      settings[`${clientToUse}Options`].messageOptions.systemMessage;
  }
}
...
async function loadSettings() {
  ...
  conversationData =
    settings.cliOptions?.conversationData || settings.conversationData || {};

  responseData = {};

  // Initialize conversation state with valid IDs
  if (!conversationData.conversationId) {
    conversationData.conversationId = crypto.randomUUID();
  }

  // Ensure we have a valid local conversation object
  localConversation = {
    messages: [],
    createdAt: Date.now(),
  };
  ...
  if (clientToUse === "bing") {
    console.log(
      tryBoxen("Welcome to the Bingleton Backrooms CLooI", {
        title: "😊",
        padding: 0.7,
        margin: 1,
    ...
    const claudeAscii = fs.readFileSync(
      "./contexts/claudeLoomAscii.txt",
      "utf8"
    );
    console.log(
      tryBoxen(claudeAscii, {
        padding: 0,
        margin: 1,
        borderStyle: "none",
        float: "center",
  ...
  // Save initial conversation state to cache
  await client.conversationsCache.set(
...
async function hasChildren() {
  ...
  return (
    getChildren(localConversation.messages, conversationData.parentMessageId)
      .length > 0
  );
}
...
async function hasSiblings() {
  ...
  return (
    getSiblings(localConversation.messages, conversationData.parentMessageId)
      .length > 1
  );
}
...
let availableCommands = [
  ...
  },
  {
    ...
    usage: "!gen",
    description:
      "Generate a response without sending an additional user message",
  ...
  },
  {
    ...
    command: async (args) =>
      rewind(
        args[1] ? parseInt(args[1], 10) : null,
        args[2] ? parseInt(args[2], 10) : null
      ),
  },
  {
    ...
    command: async (args) =>
      selectChildMessage(args[1] ? parseInt(args[1], 10) : null),
  },
  {
    ...
    command: async (args) =>
      selectSiblingMessage(args[1] ? parseInt(args[1], 10) : null),
  },
  {
    ...
    usage: "!w",
    description:
      "Navigate to the parent message. Equivalent to running !rw -1.",
    available: async () => Boolean(conversationData.parentMessageId),
    ...
    command: async () => {
      ...
      return selectSiblingMessage(
        getSiblingIndex(
          ...
          conversationData.parentMessageId
        ) + 1
      );
    },
  },
  ...
  },
  {
    ...
    available: async () => Boolean(conversationData.parentMessageId),
    command: async (args) =>
      editMessage(conversationData.parentMessageId, args.slice(1)),
  },
  {
  ...
  },
  {
    ...
    available: async () => {
      const lastConversation = await client.conversationsCache.get(
        "lastConversation"
      );
  ...
  {
    name: "!context - Select a system prompt from the contexts folder",
    value: "!context",
    usage: "!context",
    description:
      "List and select a system prompt file from the contexts folder to use as the new system message.",
    command: async () => selectSystemPrompt(),
  },
];
...
async function selectSystemPrompt() {
  ...
  try {
    ...
    if (contextFiles.length === 0) {
      logWarning("No context files found in the contexts folder.");
      return conversation();
    }
    ...
    const { selectedFile } = await inquirer.prompt([
      {
        type: "list",
        name: "selectedFile",
        message: "Select a system prompt file:",
        choices: contextFiles,
      },
    ]);

    const filePath = path.join(contextsDir, selectedFile);
    const content = await readFile(filePath, "utf8");

    // Update the system message in the client options
    clientOptions.messageOptions = clientOptions.messageOptions || {};
    clientOptions.messageOptions.systemMessage = content.trim();

    // Update the client with the new system message
    client.setOptions({
      messageOptions: {
    ...
    // Reset conversation state with new IDs
    ...
    await client.conversationsCache.set(newConversationId, localConversation);

    logSuccess(`System prompt updated to: ${selectedFile}`);
    console.log(systemMessageBox(clientOptions.messageOptions.systemMessage));
  } catch (error) {
    logError(`Error reading context files: ${error.message}`);
    return conversation();
  }
...
async function showCommandDocumentation(command) {
  if (command) {
    const commandObj = availableCommands.find(
      (c) => c.value === command || c.value === `!${command}`
    );
    if (!commandObj) {
...
async function conversation() {
  ...
  if (conversationData.parentMessageId) {
    ...
    userSuggestions =
      client.constructor.getUserSuggestions(targetMessage?.details?.message) ||
      [];
  }
  ...
  prompt.ui.activePrompt.opt.source = async (answers, input) => {
    ...
    prompt.ui.activePrompt.opt.suggestOnly =
      (!input.startsWith("!") || input.split(" ").length > 1) &&
      !input.startsWith("?");

    availableCommands = await Promise.all(
      availableCommands.map(async (command) => ({
        ...command,
    ...
    return [
      ...availableCommands.filter(
        (command) => command.isAvailable && command.value.startsWith(input)
      ),
      ...userSuggestionCommands.filter((command) =>
        command.name.toLowerCase().startsWith(input.toLowerCase())
      ),
    ];
  ...
  try {
    const context = await client.yieldGenContext(
      ...
      },
      {
        ...
        systemMessage: clientOptions.messageOptions.systemMessage,
      }
    );

    if (!context) {
      console.error("No context returned from yieldGenContext");
      return conversation();
    }
    ...
    // Ensure we have a valid parentMessageId
    const parentMessageId = completionParentId || crypto.randomUUID();
    ...
    try {
      ...
      const { results, replies } = await client.callAPI(apiParams, {
        ...clientOptions.messageOptions,
        ...
        onProgress: (diff, idx, data) => {
          if (diff) {
            ...
            if (idx === previewIdx) {
              const output = aiMessageBox(
                replaceWhitespace(streamedMessages[idx].trim())
              );
      ...
      if (!streamedMessages[previewIdx]) {
        ...
        spinner.stop();
        // Track if we've found our preview message
        let previewMessage = null;
        const newConversationMessages = [];

        for (const [key, text] of Object.entries(replies)) {
          const simpleMessage = client.buildMessage(
            text.trim(),
            client.names.bot.author
          );
          const conversationMessage = client.createConversationMessage(
            simpleMessage,
            parentMessageId
          );
          ...
          // Use Number instead of parseInt for more reliable conversion
          if (Number(key) === previewIdx) {
            previewMessage = conversationMessage;
          }
        }

        // Fallback if we didn't find a preview message
        if (!previewMessage && newConversationMessages.length > 0) {
          previewMessage = newConversationMessages[0];
        }
        ...
        if (!previewMessage) {
          console.warn("No preview message found, returning to conversation");
          return conversation();
        }
...
function getMessageByIndex(pathIndex = null, branchIndex = null) {
  ...
  // const messages = await conversationMessages();
  const siblingMessages = getSiblings(
    localConversation.messages,
    anchorMessage.id
  );
  const anchorSiblingIndex = getSiblingIndex(
    localConversation.messages,
    anchorMessage.id
  );
  if (branchIndex < 0) {
...
async function rewind(idx, branchIndex = null) {
  ...
  if (!idx) {
    const choices = messageHistory.map((conversationMessage, index) => ({
      name: `[${index}] ${conversationMessage.role}: ${
        conversationMessage.message.slice(0, 200) +
        (conversationMessage.message.length > 200 ? "..." : "")
      }`,
      value: index,
    }));
...
async function selectChildMessage(index = null) {
  // const messages = await conversationMessages();
  const childMessages = getChildren(
    localConversation.messages,
    conversationData.parentMessageId
  );
  ...
  if (index === null) {
    const choices = childMessages.map((conversationMessage, idx) => ({
      name: `[${idx}] ${conversationMessage.role}: ${
        conversationMessage.message.slice(0, 200) +
        (conversationMessage.message.length > 200 ? "..." : "")
      }`,
      value: idx,
...
async function concatMessages(newMessages) {
  ...
  const { conversationId, messageId } = await client.addMessages(
    convId,
    newMessages,
    conversationData.parentMessageId,
    true
...
async function mergeUp() {
  ...
  logSuccess(
    `Merged message ${currentMessage.id} into parent message ${parentMessage.id} and created new message ${newMessage.id}.`
  );
...
async function saveConversationState(name = null, data = conversationData) {
  ...
  const savedConversations =
    (await client.conversationsCache.get("savedConversations")) || [];
    ...
    const { overwrite } = await inquirer.prompt([
      {
        ...
        name: "overwrite",
        message:
          "A savepoint with this name already exists. Do you want to overwrite it?",
        default: false,
      },
    ]);
    if (!overwrite) {
...
async function loadByTree() {
  const conversationsWithSavedStates = await savedStatesByConversation(
    client.conversationsCache
  );
  const { conversationId } = await inquirer.prompt([
    {
      ...
      choices: Object.entries(conversationsWithSavedStates).map(
  ...
  const savedStatesInTree = conversationsWithSavedStates[
    conversationId
...
function conversationStart() {
  console.log(
    tryBoxen(`Start of conversation ${getConversationId()}`, {
      padding: 0.7,
      margin: 2,
...
function navButton(node, idx, mainMessageId) {
  //, savedIds = []) {
  ...
  buttonString =
    node.id === mainMessageId ? `[${buttonString}]` : `${buttonString}`;
  return buttonString;
...
function conversationMessageBox(conversationMessage, index = null) {
  ...
  const userMessage = Boolean(
    conversationMessage.role === client.names.user.display
  );
  const indexString = index !== null ? `[${index}] ` : "";
  // const savedIds = await getSavedIds(client.conversationsCache);
  const childrenString =
    children.length > 0
      ? ` ── !fw [${children.map((child, idx) => `${idx}`).join(" ")}]`
      : "";
  const siblingsString =
    siblings.length > 1
      ? ` ── !alt ${siblings
          .map((sibling, idx) =>
            navButton(sibling, idx, conversationMessage.id)
          )
          .join(" ")}`
      : "";
...
function historyBoxes() {
  ...
  return messageHistory
    ?.map((conversationMessage, index) =>
      conversationMessageBox(conversationMessage, index)
    )
    .join("\n");
}
...
function showHistory() {
  ...
  if (boxes) {
    ...
    if (clientToUse === "bing" && settings.cliOptions?.showSuggestions) {
      const targetMessage = getMessageByIndex();
      const suggestedUserMessages = client.constructor.getUserSuggestions(
        targetMessage.details?.message
      );
...
function getCurrentMessage() {
  ...
  return messages.find(
    (message) => message.id === conversationData.parentMessageId
  );
}
...
async function getCachedConversation() {
  ...
  const _conversation = await client.conversationsCache.get(
    getConversationId()
  );
...
function getHistory() {
  ...
  const messageHistory = getMessagesForConversation(
    localConversation.messages,
    conversationData.parentMessageId
  );
