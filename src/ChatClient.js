...
export default class ChatClient {
  ...
  setOptions(options) {
    ...
    const modelInfo =
      this.modelInfo[this.modelOptions.model] ||
    ...
    this.maxResponseTokens =
      this.modelOptions.max_tokens || modelInfo.maxResponseTokens || 400;
    ...
    if (options.messageOptions?.systemMessage) {
      this.options.messageOptions = this.options.messageOptions || {};
      this.options.messageOptions.systemMessage =
        options.messageOptions.systemMessage;
    }
  ...
  async buildConversationHistory(conversationId, parentMessageId = null) {
    ...
    const parentId =
      parentMessageId ||
      previousMessages[conversation.messages.length - 1]?.id ||
      crypto.randomUUID();
  ...
  async yieldGenContext(userMessage, modelOptions = {}, opts = {}) {
    ...
    let { conversationId, systemMessage } = opts;

    const { saveToCache = false, parentMessageId } = opts;

    if (conversationId === null) {
      conversationId = crypto.randomUUID();
    }

    const { parentId, previousMessages, conversation } =
      await this.buildConversationHistory(conversationId, parentMessageId);

    if (typeof systemMessage === "string" && systemMessage.length) {
      systemMessage = this.buildMessage(
        systemMessage,
        this.participants.system.author
      );
    }
    if (typeof userMessage === "string" && userMessage.length) {
      userMessage = this.buildMessage(
        userMessage,
        this.participants.user.author
      );
    }
    ...
    const completionParentId = userConversationMessage
      ? userConversationMessage.id
      : parentId;

    const apiParams = {
      ...modelOptions,
      ...this.buildApiParams(userMessage, previousMessages, systemMessage, {
        ...modelOptions,
        ...opts,
      }),
    };
  ...
  async sendMessage(message, modelOptions = {}, opts = {}) {
    ...
    if (opts.saveToCache) {
      for (const text of Object.values(replies)) {
        const simpleMessage = this.buildMessage(
          text.trim(),
          this.participants.bot.author
        );
        newConversationMessages.push(
          this.createConversationMessage(simpleMessage, completionParentId)
        );
      }
  ...
  async standardCompletion(messages = {}, modelOptions = {}, opts = {}) {
    const { userMessage, previousMessages, systemMessage } = messages;
  ...
  onProgressIndexical(message, replies, idx, opts) {
    ...
    if (message.delta?.text !== undefined) {
      token = message.delta.text;
    } else if (message?.choices) {
      token = this.isChatGptModel
        ? message?.choices[0]?.delta?.content
        : message?.choices[0]?.text;
    }

    if (idx !== undefined && token) {
    ...
    if (message.delta?.stop_reason) {
      opts.onFinished(idx, null, message.delta.stop_reason);
  ...
  parseReplies(result, replies) {
    Array.from(result.choices).forEach((choice, index) => {
      replies[index] = this.isChatGptModel
  ...
  async callAPI(params, opts = {}) {
    ...
    const completion = async (onProgress) =>
      this.getCompletion(
        modelOptions,
    ...
    if (n) {
      ...
      result = await completion((message) =>
        this.onProgressWrapper(message, replies, opts)
      );
    }
    if (!modelOptions.stream) {
  ...
  async getCompletion(modelOptions, onProgress, abortController, debug = true) {
    const opts = {
      method: "POST",
      headers: {
  ...
  async addMessages(
    ...
    const messageId =
      newConversationMessages[newConversationMessages.length - 1].id;
  ...
  toBasicMessage(conversationMessage) {
    const author = this.convertAlias(
      "display",
      "author",
    ...
    return {
      ...
      type:
        ...
        this.participants[author]?.defaultMessageType ||
        "message",
    };
  }
  ...
  toMessages(history) {
    switch (conversions.getDataType(history)) {
      case "[basicMessage]":
        return history;
      case "transcript":
        return conversions.parseTranscript(history);
      // case 'xml': return conversions.parseXml(history);
      case "basicMessage":
        return [history];
      case "conversationMessage":
        return [this.toBasicMessage(history)];
      case "[conversationMessage]":
        return history.map((message) => this.toBasicMessage(message));
      case "xml":
      case "string":
        return [{ text: history, author: this.participants.user.author }];
      default:
        return [];
